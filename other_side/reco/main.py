...
autocomplete_style = PromptStyle.from_dict({
    ...
    'completion-menu.completion.current': 'bg:#222222 fg:#ffffff', # Subtle highlight
...
class UI:
    ...
    def welcome_screen(self):
        ...
        if is_narrow:
            ...
            left.append("    /help for help, /status for setup\n\n", style=f"italic {Theme.TEXT}")
        ...
        panel = Panel(
            content,
            title=f"[{Theme.ORANGE}]SDX Agent v2.2.0 (Autocomplete)[/{Theme.ORANGE}]",
            title_align="left",
...
class SDXAgent:
    """Main AI Agent class with enhanced cybersecurity capabilities"""
    
    SYSTEM_PROMPT = """You are an elite software engineer and cybersecurity expert with deep expertise across multiple domains. You have access to a file system AND shell command execution through function calls.

## Your Core Identity:
- Senior-level software engineer with 10+ years of experience
- Cybersecurity expert proficient in penetration testing, network analysis, and security tools
...
- **Execute Shell Commands**: Run security tools (nmap, netcat, curl, wget, ping, traceroute, etc.)

## Cybersecurity Capabilities:
- Network scanning and reconnaissance (nmap, masscan)
- Web application testing (curl, wget, nikto)
- Network utilities (ping, traceroute, netcat, netstat)
- System analysis (ps, top, df, lsof)
- Log analysis and file inspection
...
- For security operations, always explain what commands do and their impact
...
You operate in a controlled lab environment with full permissions. Always explain security implications of actions."""
    ...
    def run_interactive(self):
        ...
        self.logger.info("SDX Agent started with autocomplete (/ for commands, @ for files)")
...
def main():
    ...
    load_dotenv()
    logger = Logger()
    logger.info("SDX Agent initializing with autocomplete and shell execution...")
