...
def show_diff(file_path: str, old_content: str, new_content: str):
    """Show a clean, sequential diff with proper gutter/content separation."""
    ...
    # Create table with fixed-width gutter and flexible content column
    grid = Table.grid(padding=(0, 1, 0, 0), expand=False)
    # Column 0: Line numbers (fixed width, right-aligned)
    grid.add_column(width=6, justify="right", no_wrap=True, style="dim")
    # Column 1: Prefix (-, +, or space)
    grid.add_column(width=1, no_wrap=True)
    # Column 2: Content (wraps properly without overlapping gutter)
    ...
    for tag, i1, i2, j1, j2 in matcher.get_opcodes():
        ...
        for i in range(start_context, i1):
            if i not in displayed_old_lines:
                line_num = Text(str(i + 1))
                prefix = Text(" ")
                content = Text(old_lines[i] if i < len(old_lines) else "", style="dim")
                grid.add_row(line_num, prefix, content)
        ...
        if tag in ('insert', 'replace'):
            for j in range(j1, j2):
                line_num = Text(str(j + 1))
                prefix = Text("+", style="bold green")
                content = Text(
                    new_lines[j] if j < len(new_lines) else "",
                    style="green on rgb(0,64,0)"
                )
                grid.add_row(line_num, prefix, content)
        ...
        if i2 < len(old_lines):
            ...
            if j2 < len(new_lines):
                ...
                for j in range(j2, end_context):
                    line_num = Text(str(j + 1))
                    prefix = Text(" ")
                    content = Text(new_lines[j] if j < len(new_lines) else "", style="dim")
                    grid.add_row(line_num, prefix, content)
